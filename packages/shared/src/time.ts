/**
 * Instants, as the server stores them. Every timestamp in the database is
 * `timestamptz` (SPEC §5), so these deal in absolute instants; a calendar day is
 * only ever a day *for somebody*, which is why the day-range helper takes the
 * offset explicitly rather than reading the machine's own timezone.
 *
 * Distinct from `dates.ts`, which is the client's local clock. These are run by
 * the server and by the app's demo backend standing in for it.
 */
